...
1. **РЕЕСТРЫ (общий реестр ИД, реестры документов качества)**
   - ✅ ВСЕГДА используют шаблоны из `docs/technical/reference/04_Шаблоны_Формы`
...
1. **Регламент на подготовку АОСР**
   - Файл: `docs/technical/reference/02_Регламенты_Процессы/02_ТЗ на подготовку АОСР.xlsx`
   - Содержит: Правила создания актов освидетельствования скрытых работ

2. **Регламент на проверку исполнительных схем**
   - Файл: `docs/technical/reference/02_Регламенты_Процессы/02_ТЗ на подготовку исполнительных схем.xlsx`
   - Содержит: Правила проверки исполнительных схем

3. **Шаблон формы АОСР**
   - Файл: `docs/technical/reference/04_Форма АОСР.xlsx`
   - Содержит: Стандартная форма для генерации АОСР в Excel

4. **Примеры итоговой ИД**
   - `docs/technical/reference/01_ИД_Примеры/01_Пример ИД на Благоустройство.pdf`
   - `docs/technical/reference/01_ИД_Примеры/01_Пример ИД на монтаж перегородок.pdf`
   - `docs/technical/reference/01_ИД_Примеры/01_Пример ИД на монтаж фунтдаментной плиты.pdf`
...
### Этап 0: Получение выбранных шаблонов

**ВАЖНО:** Шаблоны уже были выбраны пользователем ранее - сразу после анализа РД (см. раздел 3.1 в [06-user-actions-breakdown.md](06-user-actions-breakdown.md#31-выбор-шаблонов-для-генерации-документов)).

На этом этапе платформа просто получает пути к выбранным шаблонам из базы данных.

#### Функция получения шаблонов:
...
def get_project_templates(project_id: int) -> dict:
    ...
    Шаблоны уже были выбраны пользователем после анализа РД (раздел 3.1).
    Эта функция просто получает их из базы данных.

    Returns:
        dict: Словарь с путями к 3 шаблонам
        {
            'general_registry': 'путь/к/шаблону/общего/реестра.xlsx',
            'quality_registry': 'путь/к/шаблону/реестра/ДК.xlsx',
            'aosr_form': 'путь/к/шаблону/формы/АОСР.xlsx'
        }
    """
    ...
    # Проверяем, были ли выбраны шаблоны
    if not project.custom_templates:
        raise ValueError(
            f"Для проекта {project_id} не выбраны шаблоны. "
            "Пользователь должен выбрать шаблоны после анализа РД."
        )
...
```json
// project.custom_templates содержит:
{
...
def generate_registry_excel(
    ...
    if os.path.exists(template_path):
        ...
        raise FileNotFoundError(
            ...
            "Пожалуйста, загрузите шаблон реестра или используйте стандартный шаблон из папки reference/04_Шаблоны_Формы"
        )
...
def generate_aosr_excel(
    ...
    if os.path.exists(template_path):
        ...
        raise FileNotFoundError(
            ...
            "Пожалуйста, загрузите шаблон АОСР или используйте стандартный шаблон из папки reference/04_Шаблоны_Формы"
        )
