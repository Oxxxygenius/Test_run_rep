...
## ⚠️ ВАЖНО: Работа с шаблонами и программная генерация

**Принципиальное различие:**

1. **РЕЕСТРЫ (общий реестр ИД, реестры документов качества)**
   - ✅ ВСЕГДА используют шаблоны из `docs/technical/info/04_Шаблоны_Формы`
   - ❌ НИКОГДА не создаются программно
   - 🔧 При отсутствии шаблона → запросить у пользователя новый шаблон

2. **ДОКУМЕНТЫ КАЧЕСТВА (паспорта, сертификаты)**
   - ✅ Агент создает программно, если документ не найден
...
def generate_registry_excel(
    ...
    if os.path.exists(template_path):
        ...
        raise FileNotFoundError(
            f"Шаблон общего реестра не найден: {template_path}\n"
            "Пожалуйста, загрузите шаблон реестра или используйте стандартный шаблон из папки 04_Шаблоны_Формы"
        )
...
def generate_quality_docs_registry_excel(package: AOSRPackage, output_path: str) -> str:
    ...
    if os.path.exists(template_path):
        ...
        # Если шаблона нет - запрашиваем у пользователя
        raise FileNotFoundError(
            f"Шаблон реестра документов качества не найден: {template_path}\n"
            "Пожалуйста, загрузите шаблон реестра или используйте стандартный шаблон из папки 04_Шаблоны_Формы"
        )
