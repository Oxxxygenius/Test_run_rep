...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│ 1️⃣ Автоматическое определение комплекта документов                    │
│    • GPT-4o анализирует название материала                             │
│    • Определяет категорию (ГКЛ, краски, трубы, крепёж, и т.д.)         │
│    • Загружает список требуемых документов из таблицы material_categories│
│    • Не каждый материал требует полный набор документов                │
│                                                                         │
│ 2️⃣ Сертификаты, СРГ и Пожарные сертификаты — ТОЛЬКО из надёжных       │
│    источников                                                          │
│    • Специализированные сайты (приоритет!)                             │
│    • Официальные реестры (fsa.gov.ru, rospotrebnadzor.ru)              │
│    • Google с тщательной проверкой GPT-4o                              │
│    • AI генерация ЗАПРЕЩЕНА для всех официальных документов            │
│                                                                         │
│ 3️⃣ Паспорта качества — ГЕНЕРАЦИЯ ПРИОРИТЕТНА                          │
│    • Если есть Сертификат → генерируем Паспорт из него                 │
│    • Если НЕТ Сертификата → генерация ЗАПРЕЩЕНА                        │
│    • Помечаем: "Сгенерировано на основе Сертификата №..."              │
│    • Уверенность: 80-90% (требует проверки)                            │
│                                                                         │
│ 4️⃣ Отказные письма — ГЕНЕРАЦИЯ РАЗРЕШЕНА                              │
│    • Если материал не подлежит обязательной сертификации               │
│    • Генерируем от лица организации-поставщика                         │
│    • На бланке организации с реквизитами                               │
│    • Помечаем: "ТРЕБУЕТСЯ ПРОВЕРКА И ПОДПИСЬ"                          │
│    • Пользователь должен проверить, подписать и загрузить обратно      │
│    • Уверенность: 90%                                                  │
│                                                                         │
│ 5️⃣ Технические паспорта — сайты производителей                        │
│    • Искать на официальных сайтах                                      │
│    • Google как альтернатива                                           │
│    • AI генерация возможна (типовой из ГОСТа)                          │
│    • Используются редко (в основном на стальные трубы)                 │
│                                                                         │
│ 6️⃣ Прозрачность для пользователя                                       │
│    • Каждый документ помечен источником                                │
│    • Для сгенерированных — ссылка на исходный Сертификат               │
│    • Уровень уверенности отображается                                  │
│    • Видно какие документы требуют подписи (отказные письма)           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
