...
```
┌─────────────────────────────────────────────────────────────────────┐
│ УМНАЯ ГЕНЕРАЦИЯ С ПРОВЕРКОЙ ЗАВИСИМОСТЕЙ                            │
└─────────────────────────────────────────────────────────────────────┘

Celery Queue      AI Worker       PostgreSQL      GPT-4o API
────────────      ─────────       ──────────      ──────────
     │                 │               │               │
     │ Task 1:         │               │               │
     │ "Паспорт        │               │               │
...
### 6. Умная генерация
- **Сертификаты и СРГ** — только из интернета (безопасно)
- **Паспорта качества** — генерируются из найденных Сертификатов
- **Зависимости учтены** — если нет Сертификата, Паспорт не генерируем
- **Прозрачность** — видно откуда данные (из какого Сертификата)
